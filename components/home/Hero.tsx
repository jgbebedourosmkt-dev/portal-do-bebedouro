  ...
  return (
    <section className="w-full" aria-label="Destaque principal">
      ...
      >
        ...
        >
          ...
          >
            ...
            <Link href={`/artigo/${featured.slug}`}>
              <h2
                className="text-[41px] font-black text-white leading-tight hover:text-acc transition-colors line-clamp-3"
                style={{ fontFamily: 'var(--font-barlow-condensed)' }}
              >
                {featured.title}
              </h2>
            </Link>
            <p className="mt-2 text-[17px] text-white/70 line-clamp-2 max-w-xl">
              {featured.excerpt}
            </p>
            <div className="mt-3 flex items-center gap-3 text-[14px] text-white/50">
        ...
        <div className="grid grid-cols-2 border-l border-white/10 bg-black" style={{ gridTemplateRows: '1fr 1fr' }}>
          {secondary.slice(0, 4).map((post) => (
            <div key={post.slug} className="relative flex flex-col justify-end p-4 overflow-hidden border-b border-r border-white/10">
              {post.ogImage ? (
                <Image
                  src={post.ogImage}
                  alt={post.title}
              ...
              <div className="relative z-10">
                ...
                <Link href={`/artigo/${post.slug}`}>
                  <h3
                    className="mt-1 text-[18px] font-black text-white leading-tight hover:text-acc transition-colors line-clamp-3"
                    style={{ fontFamily: 'var(--font-barlow-condensed)' }}
                  >
                    {post.title}
                  </h3>
